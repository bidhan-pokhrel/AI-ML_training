...
BEFORE RUNNING THIS FILE:
  1. Run the notebook Credit_Score_12_Steps.ipynb first
  2. Make sure credit_model.pkl is in this same folder
...
if os.path.exists("credit_model.pkl") == False:
    st.error("credit_model.pkl not found in this folder.")
    st.info("Run the notebook Credit_Score_12_Steps.ipynb first (Step 12 creates this file).")
    st.stop()

package = joblib.load("credit_model.pkl")
...
st.set_page_config(page_title="Credit Score Ranking", page_icon="🏦")

st.title("🏦 Is this customer Poor, Average or Good?")
...
monthly_income = st.slider("Monthly income", 12000, 300000, 60000)
loan_amount = st.slider("Loan amount", 10000, 2000000, 150000)
existing_loans = st.slider("Existing loans", 0, 5, 0)
missed_payments = st.slider("Missed payments", 0, 9, 0)
credit_history_years = st.slider("Years of credit history", 0, 25, 8)
...
new_row = pd.DataFrame([[age, monthly_income, loan_amount, existing_loans, missed_payments, credit_history_years]], columns=columns)
...
if prediction == "Good":
    st.success("✅  GOOD - low risk, the loan can normally be approved")
elif prediction == "Average":
    st.warning("🟡  AVERAGE - medium risk, approve with conditions")
else:
    st.error("❌  POOR - high risk, a guarantor is recommended")
...
#  6. Set Main file path to:  credit_app.py
