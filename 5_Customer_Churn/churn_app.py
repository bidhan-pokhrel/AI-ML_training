...
BEFORE RUNNING THIS FILE:
  1. Run the notebook Customer_Churn_12_Steps.ipynb first
  2. Make sure churn_model.pkl is in this same folder
...
if os.path.exists("churn_model.pkl") == False:
    st.error("churn_model.pkl not found in this folder.")
    st.info("Run the notebook Customer_Churn_12_Steps.ipynb first (Step 12 creates this file).")
    st.stop()
...
st.title("🎯 Will this customer leave us?")
...
tenure_months = st.slider("Months as a customer", 1, 60, 10)
monthly_charges = st.slider("Monthly charges", 299, 1999, 1200)
total_spent = st.slider("Total spent so far", 300, 100000, 12000)
support_calls = st.slider("Support calls", 0, 12, 5)
late_payments = st.slider("Late payments", 0, 8, 3)
...
new_row = pd.DataFrame([[age, tenure_months, monthly_charges, total_spent, support_calls, late_payments]], columns=columns)
...
if prediction == "Yes":
    st.error("⚠️  THIS CUSTOMER WILL LEAVE - send a retention offer today")
else:
    st.success("✅  THIS CUSTOMER WILL STAY - no action needed")
...
st.metric("Risk of leaving", str(round(chance * 100, 2)) + " %")
...
#  6. Set Main file path to:  churn_app.py
